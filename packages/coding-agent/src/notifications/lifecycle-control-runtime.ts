...
export function daemonResumeSession(env: NodeJS.ProcessEnv = process.env, opts: { sessionsRoot?: string } = {}) {
	return async (target: {
		...
		if (opts.sessionsRoot) {
			...
			resumeCwd = selected.path ?? resumeCwd;
			if (!resumeCwd) throw new Error("gjc_lifecycle_resume_missing_cwd");
		}
		const resolvedResumeCwd = resumeCwd ? path.resolve(resumeCwd) : undefined;
		if (resolvedResumeCwd) {
			const stat = fs.statSync(resolvedResumeCwd, { throwIfNoEntry: false });
			if (!stat?.isDirectory()) throw new Error(`gjc_lifecycle_resume_cwd_unavailable: ${resolvedResumeCwd}`);
		}
		const tmux = resolveGjcTmuxCommand(env);
